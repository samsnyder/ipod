...
class MKTunes(itunesDir: File, artworkDb: ArtworkDB, ipodName: String,
  ...
  def writeAllPlaylists(out: ByteBuffer, tracks: Array[LibTrack],
    ...
    val masterPlaylist = LibPlaylist(ipodName, allTrackIds)
    createPlaylist(masterBuffer, masterPlaylist, isHidden = true, MPL_UID)
    masterBuffer.flip
    ...
    for(playlist <- playlists){
      ...
      createPlaylist(playlistsBuffer, playlist, isHidden = false, playlistId)
    }
