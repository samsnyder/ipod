...
object iTunesDB {
  ...
  def mkMhod(out: ByteBuffer, key: String, value: String): Boolean =
    mkMhod(out, key, value, 1, isPlaylist = false)
  def mkMhod(out: ByteBuffer, playlistId: Int): Boolean =
    mkMhod(out, null, null, playlistId, isPlaylist = true)
  ...
  def mkMhyp(out: ByteBuffer, size: Int, name: String, playlistType: Int, fileNum: Int,
    ...
    mkMhyp(out, size, name, playlistType, fileNum, playlistId, mhodCount, isPodcast = false)

  def mkMhyp(out: ByteBuffer): Unit = mkMhyp(out, 0, "Podcasts", 0, 0, 0, 0, isPodcast = true)
